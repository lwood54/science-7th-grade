...
const NewGameTemplate = (props) => {
  ...
  const newCards = useCards(props.game, handleDrag, handleCardClick, shuffleArray);
  ...
  const cardsToShuffle = useCards(useCards(props.game, handleDrag, handleCardClick, shuffleArray));
  const updatedTargets = useTargets(handleDragOver, handleDrop, handleTargetClick);
  useEffect(() => {
    if (canRestart) {
        ...
        <DialogContent>
          {calculatedScore >= 70 ? (
            <div className={cls.VictoryContainer}>
              <h2>You passed this section with a score of {calculatedScore}!!! Great job!!!</h2>
              <img src={keep_going} className={cls.Image} alt="keep going Homer Simpson" />
            </div>
          ) : (
            <h2>Sorry, you got a score of {calculatedScore}. Go ahead and retry this section.</h2>
          )}
        </DialogContent>
        <DialogActions>
          {restart}
