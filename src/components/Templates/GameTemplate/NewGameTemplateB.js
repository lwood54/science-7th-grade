...
const NewGameTemplateB = (props) => {
  ...
  const updatedCards = useCards(props.game2, handleDrag, handleCardClick, shuffleArray);
  useEffect(() => {
  ...
  const removeCards = () => {
    for (let k = 0; k < targetsRef.current.children.length; k++) {
      for (let j = 0; j < targetsRef.current.children[k].children.length; j++) {
        let nestedCards = targetsRef.current.children[k].children[j].children;
        if (nestedCards[0]) {
          let removedChild = targetsRef.current.children[k].children[j].removeChild(nestedCards[0]);
          targetsRef.current.children[k].children[j].classList.remove(cls.CorrectTarget, cls.IncorrectTarget);
  ...
  const cardsToShuffle = useCards(props.game2, handleDrag, handleCardClick, shuffleArray);
  const updatedTargets = useTargets(handleDragOver, handleDrop);
  useEffect(() => {
    if (canRestart) {
      // console.log('canRestart is running');
      let shuffledCards = shuffleArray(cardsToShuffle);
  ...
  return (
    <div style={{ width: "100%" }}>
      <NavigationBar title={props.title} quizletLink={navQuizlet} homeLink={navHome} unitMain={navUnit} />
      <Dialog
        open={isRoundOver}
        ...
        <DialogContent>
          {calculatedScore >= 70 ? (
            <div className={cls.VictoryContainer}>
              ...
              <img src={victory_royale} className={cls.Image} alt="victory royale" />
            </div>
          ) : (
            <h2>Sorry, you got a score of {calculatedScore}. What would you like to do?</h2>
          )}
        </DialogContent>
        <DialogActions>
      ...
      <div className={cls.NewGameContainer}>
        <div className={cls.NewGameBoard} ref={targetsRef}>
          <div className={cls.NewCol}>
            <h1 className={cls.NewColTitle}>{props.game2.col1.heading}</h1>
            {targets[0]}
            {targets[1]}
            {targets[2]}
          </div>
          <div className={cls.NewCol}>
            <h1 className={cls.NewColTitle}>{props.game2.col2.heading}</h1>
            {targets[3]}
            {targets[4]}
            {targets[5]}
          </div>
          <div className={cls.NewCol}>
            <h1 className={cls.NewColTitle}>{props.game2.col3.heading}</h1>
            {targets[6]}
            {targets[7]}
            {targets[8]}
          </div>
          <div className={cls.NewCol}>
            <h1 className={cls.NewColTitle}>{props.game2.col4.heading}</h1>
            {targets[9]}
            {targets[10]}
            {targets[11]}
          </div>
          <div className={cls.NewCol}>
            <h1 className={cls.NewColTitle}>{props.game2.col5.heading}</h1>
            {targets[12]}
            {targets[13]}
            {targets[14]}
          </div>
