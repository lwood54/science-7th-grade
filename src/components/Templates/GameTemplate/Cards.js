...
const useCards = (game, handleDrag, handleCardClick) => {
  let newCards = [];
  ...
  arrayOfCols.forEach((col) => {
    ...
    itemsArray.forEach((item) => {
      if (item === "image") {
        ...
        newCards.push(
          <div className={cls.NewCard} key={`${col}_${item}`} id={`${col}_${item}`} onDragStart={handleDrag} draggable onClick={handleCardClick}>
            <img src={gameCopy[col][item]} alt="game piece" className={cls.Image} draggable="false" />
          </div>
        );
        ...
        newCards.push(
          <div key={`${col}_${item}`} id={`${col}_${item}`} className={cls.NewCard} onDragStart={handleDrag} draggable onClick={handleCardClick}>
            {gameCopy[col][item].length < 80 ? gameCopy[col][item] : console.log(`${col} : ${item} too long by ${gameCopy[col][item].length - 80}`)}
          </div>
        );
      }
    });
